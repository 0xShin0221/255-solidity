...
/**
 *
 * ProofOfVisit: SoulboundNFT
 *
 * Based on Membership NFTs by Ezra Weller and R Group, working with Rarible DAO
 *
 */

contract ProofOfVisit is Initializable, AccessControlUpgradeable, ERC721VotesUpgradeable, ERC721BurnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  function constructTokenURI(TokenURIParams memory params) internal view returns (string memory) {
    string memory svg = Base64Upgradeable.encode(
      bytes(
        abi.encodePacked(
